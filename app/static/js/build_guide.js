...
(function($){
  ...
  function gatherSlotData(){
    ...
    $('#slotsArea').children().each(function(){
      ...
      const weap = $slot.find('[id^="wep_"]').val()?.trim() || '';
...
$('#slotsArea').css('overflow', 'visible');
$(document).on('mouseenter', '.slot-card', function () {
...
function filterLibraryItems(clickedBtn) {
  ...
  $('.library-item').each(function () {
    ...
    if (activeElement !== 'all' && element !== activeElement) {
      show = false;
    }

    if (activeWeapon && weapon !== activeWeapon) {
      show = false;
    }
...
// store selected characters (slug-based)
const selectedCharacters = new Set();

$(document).on('click', '.character-card.selectable', function () {
  const $card = $(this);
  const charId = $card.data('char-id');
...
$('#libClear').on('click', function () {
  selectedCharacters.clear();
  $('.character-card.selected').removeClass('selected');
});
...
$('#libraryModal').on('shown.bs.modal', function () {
  const user_id = $('#user_id').val();
  const saved = localStorage.getItem(`library_selection_${user_id}`);
  if (!saved) return;

  saved.split(',').forEach(id => {
    selectedCharacters.add(id);
    $(`.character-card[data-char-id="${id}"]`).addClass('selected');
  });
});
